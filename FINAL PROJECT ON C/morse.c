...
#define THREE_UNIT 3   // mean 3 spaces  
...
{                                                                                                   //parametr 0- Translate_cmf_and_print_text 
	...
	char buffer_string[LENG_BUFFER] = { NULL };    //buffer for save input 
...
void Load_text_and_print_cmf(struct morse_code* morse_translate, FILE* file)
{
	...
	{
		if (buffer_string[i] != ' ' || buffer_string[i + 1] == '\0')
		{
...
void Load_cmf_and_save_text(struct morse_code* morse_translate, FILE* file1, FILE* file2)
{
	...
	while (buffer_string[i])
	{
		while (buffer_string[i] == ' ' && buffer_string[i + 1] == ' ')
		{
			counter_revah++;
			i++;
		}
		if (buffer_string[i + 1] == '\0')
		{
			ptr_buffer_word = add_char(ptr_buffer_word, buffer_string[i], s);
			counter_revah = 3;
		}
		if (counter_revah >= THREE_UNIT)
		{
			if (character = Translate_Morse_To_TEXT(ptr_buffer_word, morse_translate)); //call func 2 args(string from buffer, ptr to struct)  
			else return;
			fprintf(file2, "%c", character);
			if (counter_revah == SEVEN_UNIT) fprintf(file2, " ");
			s = counter_revah = 1;
			ptr_buffer_word = NULL;
		}
...
void Load_text_and_save_cmf(struct morse_code* morse_translate, FILE* file1, FILE* file2)
{
	...
	while (buffer_string[i])
	{
		if (buffer_string[i] != ' ' && buffer_string[i + 1] != '\0' && buffer_string[i + 1] != ' ')
		{
			if
				(ptr_buffer_word = Translate_Text_To_Morse(ptr_buffer_word, buffer_string[i], morse_translate));// if char not exist,return to main function
			else return;
		...
		else
		{
			...
			fprintf(file2, "%s", ptr_buffer_word);
		}
		i++;
	}
}